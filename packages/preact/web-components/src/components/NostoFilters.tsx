...
import { NOSTO_EVENTS, FilterChangeEventDetail, SearchEventDetail } from "../types"

/**
 * Filter component for a single facet
 */
function FilterFacet({ facet, onFilterChange }: { facet: any; onFilterChange: (detail: FilterChangeEventDetail) => void }) {
  const { active, selectedFiltersCount, toggleActive, toggleProductFilter } = useFacet(facet)

  const handleFilterToggle = useCallback((value: string, selected: boolean) => {
  ...
  return (
    <div className="nosto-selected-filters" style={{ marginBottom: "16px" }}>
      <div style={{ fontWeight: "600", marginBottom: "8px" }}>
        Selected Filters ({selectedFiltersCount})
      </div>
...
function FiltersWrapper({ onFilterChange }: { onFilterChange: (detail: FilterChangeEventDetail) => void }) {
  ...
  const handleClearFilter = useCallback((field: string, value: string) => {
    onFilterChange({
      field,
      value,
      active: false
